...
def make_psd_profile(psd_info_list, sample_freq, epoch_len_sec, psd_type='norm', mask=None, hourly=True):
    """
    Makes summary PSD statistics of each mouse.

    Arguments:
        psd_info_list {list} -- A list of psd_info generated by make_target_psd_info()
        sample_freq {int} -- Sampling frequency
        psd_type {str} -- 'norm' or 'raw'
        mask {np.array} -- Binary mask specifying epochs to be included. Default is None to include all epochs.
        hourly {bool} -- Whether to calculate hourly PSD profiles.

    Returns:
        psd_summary_df {pd.DataFrame}
        hourly_psd_summary_df {pd.DataFrame} (if hourly=True)
    """
    ...
    print("make_psd_profile")

    # Collect rows for PSD summary
    psd_rows = []
    hourly_rows = []
    ...
    for psd_info in psd_info_list:
        ...
        # Collect rows for PSD summary
        psd_rows.extend([
            [exp_label, mouse_group, mouse_id, device_label, 'REM', np.sum(bidx_rem_target)] + psd_summary_rem.tolist(),
            [exp_label, mouse_group, mouse_id, device_label, 'NREM', np.sum(bidx_nrem_target)] + psd_summary_nrem.tolist(),
            [exp_label, mouse_group, mouse_id, device_label, 'Wake', np.sum(bidx_wake_target)] + psd_summary_wake.tolist()
        ])

        if hourly:
            epochs_per_hour = int(3600 / epoch_len_sec)
            total_epochs = len(psd_info['bidx_target'])
            total_hours = total_epochs // epochs_per_hour
            ...
            for hour in range(total_hours):
                ...
                psd_summary_wake_hourly = _psd_summary_by_bidx(bidx_wake_hourly)

                # Collect rows for hourly PSD summary
                hourly_rows.extend([
                    [exp_label, mouse_group, mouse_id, device_label, 'REM', np.sum(bidx_rem_hourly), hour] + psd_summary_rem_hourly.tolist(),
                    [exp_label, mouse_group, mouse_id, device_label, 'NREM', np.sum(bidx_nrem_hourly), hour] + psd_summary_nrem_hourly.tolist(),
                    [exp_label, mouse_group, mouse_id, device_label, 'Wake', np.sum(bidx_wake_hourly), hour] + psd_summary_wake_hourly.tolist()
                ])

    # Create PSD summary DataFrame
    freq_columns = [f'f@{x}' for x in freq_bins.tolist()]
    psd_columns = ['Experiment label', 'Mouse group', 'Mouse ID', 'Device label', 'Stage', 'epoch #'] + freq_columns
    psd_summary_df = pd.DataFrame(psd_rows, columns=psd_columns)

    if hourly:
        # Create hourly PSD summary DataFrame
        hourly_columns = ['Experiment label', 'Mouse group', 'Mouse ID', 'Device label', 'Stage', 'epoch #', 'hour'] + freq_columns
...
def make_target_psd_info(mouse_info_df, epoch_range, epoch_len_sec, sample_freq,
                         stage_ext, start_datetime,result_dir_name):
...
def make_psd_stats(psd_domain_df):
    """ Makes a table of statistical tests for each frequency domain between groups.

    Arguments:
        psd_domain_df {pd.DataFrame} -- A dataframe generated by make_psd_domain().

    Returns:
        [pd.DataFrame] -- A table with the following columns:
            Mouse group, Stage type, Wave type, N, Mean, SD, Pvalue, Stars, Method
    """
    ...
    # Control group
    group_c = mouse_group_set[0]

    # Define stages and collect control data
    ...
    for stage_name, powers_domains in zip(stage_names, powers_domains_stages_c):
        for domain_name in DOMAIN_NAMES:
            ...
            num = np.sum(~np.isnan(powers))  # Count effective N
            rows.append([
                group_c, stage_name, domain_name, num,
    ...
    for group_t in mouse_group_set[1:]:
        ...
        for stage_name, powers_domains_c, powers_domains_t in zip(stage_names, powers_domains_stages_c, powers_domains_stages_t):
            for domain_name in DOMAIN_NAMES:
                ...
                num = np.sum(~np.isnan(powers_t))  # Count effective N
                rows.append([
                    group_t, stage_name, domain_name, num,
                    np.nanmean(powers_t), np.nanstd(powers_t),
                    test['p_value'], test['stars'], test['method']
                ])
...
def make_psd_timeseries_df(psd_info_list, epoch_range, bidx_freq, stage_bidx_key=None, psd_type='norm'):
    ...
    for psd_info in psd_info_list:
        ...
        new_row = pd.DataFrame(
            [[psd_info['exp_label'], psd_info['mouse_group'], psd_info['mouse_id'], psd_info['device_label']] + psd_delta_timeseries.tolist()],
            columns=psd_timeseries_df.columns
        )
